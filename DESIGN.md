...
本文の右に何かを置くページでは、本文だけでなくページの中身全体を中央に釣り合わせる。本文を中央に置いたままだと、右に置いたぶん左の余白だけが広くなり、ページが右へ寄って見える。中央に置いた文字の軸は、ヘッダーからフッターまで 1 本に揃える。
...
下線は有無を切り替えず、色の補間で出し入れする。有無は補間できず、hover した瞬間に文字が跳ねて見える。

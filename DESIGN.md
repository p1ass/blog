...
blog.p1ass.com のデザインの思想と方針。[design.md](https://github.com/google-labs-code/design.md) 形式で、front matter がトークン、この本文がその理由にあたる。front matter は `npx @google/design.md lint DESIGN.md` で確かめる。

この本文に書くのは、複数のコンポーネントにまたがる方針だけ。個々のコンポーネントの寸法や配置は、スタイルガイド (`/styleguide`) で実物を見る。

実装の置き場所と、記事まわりの語彙は [CLAUDE.md](CLAUDE.md) にある。デザインを変える手順は [design-system Skill](.claude/skills/design-system/SKILL.md) が案内する。

## Overview

日本語の技術記事を、腰を据えて読ませるためのブログ。読者は仕事の合間に検索から流れ着くエンジニアで、目的は記事を最後まで読み通してもらうことにある。

デザインはおとなしめで、記事の内容より前に出ない。装飾ではなく、行長と行間と余白のリズムで読みやすさを作る。色数を絞り、1 色相の濃淡で組む。読者の注意を引くのは、リンクと `Note` と見出しのボーダーだけにとどめる。
...
1 色相の濃淡でできている。プリミティブは neutral (色相 216 度) と accent (色相 215 度) の 2 系統で、そこに注意と補足のための色相を 2 つだけ足してある。

- **Primary (#4172b5):** リンク、アクセント線、フォーカスリングに使う唯一のアクセント。OG 画像にも使う事実上のブランドカラーなので、値を変更しない
- **Secondary (#636e7d):** 日付やキャプションのように、本文より一段落とす文字
- **Neutral (#ffffff 〜 #0f1114):** 地と文字。12 段すべてが同じ色相で、明るいテーマと暗いテーマの両方をここから取る
- **Warning (#a35b00) / Tip (#1f7a3d):** 注意と補足を示す色。accent と同じ色相の濃淡で分けると、記事を流し読みしたときに「いつもと違う」が伝わらない

### 段の決め方

数字は明度の目安で、小さいほど明るい。

`accent-muted` には accent の 400 を当てる。リンクを色以外で見分けるための下線に使うので、UI コンポーネントと同じ 3 対 1 を満たす段を採る。1 段薄い 300 は、白地で 2.35 対 1 にとどまる。
...
読者の OS のフォントを使い、Web フォントは入れない。表示速度を保つためで、デザインの伸びしろはフォントそのものより行長と行間とサイズの整理のほうが大きい。
...
- **キャプション:** 13px。日付や補足のように、本文より一段落とす文字に使う
...
和文を先に並べる。欧文だけのフォントを先に置くと、和文がそのフォントのフォールバックに委ねられ、環境によって別のフォントで描かれる。
...
階層はボーダーと面のカラーで作る。地が `surface`、一段沈んだ面が `surface-subtle`、その境界が `border`。面を持つ要素は、この 3 つの組み合わせだけで奥行きを出す。
...
角丸は 3 段。小さい要素に 4px、面を持つ塊に 8px、円形のものに `full` を使う。

ボーダーの太さは 1px の 1 段に絞る。3px は強調の線にだけ使う。

## Components

コンポーネントごとのデザインはスタイルガイドにある。この節に置くのは、どのコンポーネントにも当てはまる方針だけ。

### リンク

リンクは色だけでなく、下線でも見分けられるようにする。文章の中に置くリンクには平常時から薄い下線を引き、単独で置くリンクは hover と focus のときにだけ下線を出す。

hover と focus では下線を太くし、背後に面を敷く。細い線の色を変えるだけでは、指しているかどうかが分からない。

文字の色は hover でも変えない。accent には、本文の基準に対するコントラストの余裕がない。

下線は有無を切り替えず、色の補間で出し入れする。有無は補間できず、hover したとたんに文字が跳ねて見える。

### フォーカス

フォーカスリングは `:focus-visible` に 1 つだけ、グローバル規則として置く。要素ごとに書くと、書き忘れた要素だけキーボードで追えなくなる。`:focus` ではなく `:focus-visible` にするのは、マウスで押した直後にリングが残るのを避けるためだ。

### 面を持つリンク

面の全体で hover を受ける。一部だけが反応すると、押せる範囲がデザインより狭く感じられる。

### 画像

画像は読み込む前から場所を確保する。寸法がないと、読み終わった時点で下の本文を押し下げる。

### モーション

色の変化と、位置や大きさの動きで扱いを分ける。

色の変化は `ease` で 150ms。hover から離れたときの戻りは 100ms にして、入るときより速くする。読者が選んでいる途中の変化はゆっくりでよいが、離れた後まで変化が残ると反応が鈍く見える。

位置と大きさの動きは、減衰比 1 のばね (Apple の定義で response 0.35 秒) を `linear()` で近似した曲線で 450ms かけて動かす。跳ね返らずに、最後にゆっくり収まる。`cubic-bezier` の ease-out を短く当てると、一瞬で止まって硬く見える。入るときと戻るときで、ばねの曲線は変えない。

動かすプロパティは名指しで書く。`transition: all` は将来足したプロパティまで巻き込むので、色を変えるつもりの hover で padding や width まで動く。位置と大きさは `transform` で動かし、`width` や `margin` は動かさない。フレームごとにレイアウトを計算し直すことになる。

hover の見た目は、カーソルを持つ端末 (`hover: hover` かつ `pointer: fine`) に限る。タッチ端末ではタップした要素に `:hover` が残り、記事から戻ったときに一覧のカードが hover したままになる。`:focus-visible` は限らない。

ボタンは押している間 `scale(0.97)` に縮める。押したことを、ページが移る前に伝える。押したときは出だしの強い `cubic-bezier(0.23, 1, 0.32, 1)` で 100ms にして即座に縮め、離したときはばねで戻す。面の大きいカードは縮めない。

一覧を開くような、たまにしか出ないものは `@starting-style` で登場させる。起点は引き金のある側に置く。閉じるときは開いたときの動きを 100ms で逆にたどる。出てきた場所へ戻ると、どこへ消えたかが分かる。

読者がテーマを選んだら、View Transitions でページ全体を 250ms のクロスフェードで切り替える。明暗が一瞬で入れ替わると目に強い。要素ごとの transition はその間止める。止めないと、transition を持つ要素だけが地より遅れて色を変える。

ページの遷移は View Transitions で 200ms のクロスフェードにする。遷移は読者が何度も繰り返す操作なので、短くし、ページ全体の位置は大きく動かさない。

ヘッダーに `view-transition-name` は付けない。前後のページで同じ位置にあるヘッダーは、重ねても画素が変わらず止まって見える。ヘッダーの高さはページによらずそろえる。高さが違うと、遷移のたびにヘッダーが伸び縮みする。

一覧から記事へ移るときは、記事のタイトル、タグ、抜粋を移る前の位置からつなぎ、位置と大きさの動きとしてばねで動かす。どの記事を開いたかを、移った先でも見失わない。日付はつながない。小さな文字が長い距離を動くと目を引くが、記事を見失わないための手がかりにはならない。名前は開く記事の要素にだけ、画面に入っているときに付ける。画面の外から飛んでくる動きは、何が動いたかが分からない。このときはページ全体のクロスフェードも同じばねにそろえる。フェードだけが先に終わると、本文が出そろった後もタイトルが動き続け、主役と周りがばらばらに見える。つなぐ要素の箱は文字の幅に合わせる。箱が本文の幅のままだと、文字が拡大されずに横へ滑る。前後の記事どうしはつながず、矢印の向きへのずれに任せる。

ページ送りと前後の記事は、押した矢印の向きへ 16px だけずらす。戻ったときは逆向きにする。

遷移先は hover したときに先読みし、アニメーションが読み込みで詰まらないようにする。

動きを減らす設定の読者には、`prefers-reduced-motion: reduce` で位置と大きさの動きだけを止める。色と透明度の補間、テーマとページのクロスフェードは酔いの原因にならず、状態の変化を伝えるので残す。

## Theming

明るいテーマと暗いテーマの 2 つを持つ。既定は OS の設定への追従で、読者は明示的に選べる。一度選んだ後も、OS への追従へ戻せるようにする。
...
読者の選択は最初の描画より前に当てる。後から当てると、リロードのたびに色が入れ替わって見える。
...
アイコンは図形をソースに書き、外部から読み込まない。外部の kit を読むと、アイコン数個のために毎回スクリプトを参照しなければならない。

ブランドマークは [Simple Icons](https://simple-icons.org) から取る。プロジェクト自体は CC0 だが、[DISCLAIMER](https://github.com/simple-icons/simple-icons/blob/develop/DISCLAIMER.md) が「収録アイコンすべてが CC0 とは限らない」と断っているとおり、マークの商標は各社に残る。使うのは各社が配布を認めている範囲にとどめる。それ以外のアイコンは自前で描く。円や三角と線だけで済む図形なら、外から持ってくる理由がない。

ブランドマークの形と色は変えない。暗い地に置くときは、各社が暗い地向けに配っている版に従う。ロゴの一部である文字は、Colors に書いた Logotypes の例外にあたる。満たす必要のない基準のために色を変えると、それが何かを見分ける手がかりを失う。

## Do's and Don'ts

- Do 値をトークンから参照する。色と寸法と分岐を、コンポーネントの側で決めない
- Do 色を足すときは明るいテーマと暗いテーマの両方を同時に決める
- Do 新しい役割を足したら、コントラストの検証にその組み合わせを足す
- Do コンポーネントひとつの都合で決まる寸法 (アバターの直径など) は生の値のまま書く。段を作っても参照する先が 1 箇所にしかない
- Don't 影を使う。階層はボーダーと面のカラーで作る
- Don't hover でリンクの文字色を動かす。コントラストの余裕がない
- Don't 状態を色だけで示す。色の見え方によっては読み取れない
- Don't `transition: all` を書く。動かすプロパティを名指しする
- Don't hover の見た目を `canHover` の外に書く。タッチ端末でタップした後に残る

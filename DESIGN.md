...
一覧から記事へ移るときは、記事のタイトル、日付、タグ、抜粋を移る前の位置からつなぎ、位置と大きさの動きとしてばねで動かす。どの記事を開いたかを、移った先でも見失わない。名前は開く記事の要素にだけ、画面に入っているときに付ける。画面の外から飛んでくる動きは、何が動いたかが分からない。このときはページ全体のクロスフェードも同じばねにそろえる。フェードだけが先に終わると、本文が出そろった後もタイトルが動き続け、主役と周りがばらばらに見える。つなぐ要素の箱は文字の幅に合わせる。箱が本文の幅のままだと、文字が拡大されずに横へ滑る。前後の記事どうしはつながず、矢印の向きへのずれに任せる。

...
ページの遷移は View Transitions で 200ms のクロスフェードにする。遷移は読者が何度も繰り返す操作なので、短く、位置を大きく動かさない。

要素に `view-transition-name` は付けない。前後のページで同じ位置にあるヘッダーは、重ねても画素が変わらず止まって見える。一覧のタイトルを記事の見出しへつなぐ動きは入れない。両者は大きさと寄せ方と折り返しが違い、どう合わせてもちらつく。ヘッダーの高さはページによらずそろえる。高さが違うと、遷移のたびにヘッダーが伸び縮みする。

...
- **Primary (#4172b5):** リンク、アクセント線、フォーカスリングに使う唯一のアクセント。OG 画像の下辺のボーダーにも使う事実上のブランドカラーなので値を変更しない。OG 画像は `scripts/generate-og-images.ts` がこのトークンを読んで生成するため、値を変えると次のビルドで 77 枚が新しい青になる
...
サムネイルは 1200×630 (1.91:1) の枠で出す。OGP の画像の事実上の標準で、リンク先の多くがこの比で画像を用意している。横長でない画像は枠いっぱいに拡大し、はみ出た部分を切る。正方形のロゴは上下が欠けるが、該当するのは 1 割ほどにとどまる。

どちらの幅でも、テキストを左、サムネイルを右に置く。

640px 以上では、サムネイルをカードの高さいっぱいに置き、カードの高さを 112px に固定する。リンクカードが続いたときに高さがそろって見えるためだ。112px はタイトル 2 行、説明 1 行、ホスト名がすべて収まる高さにあたる。画像のないカードも同じ高さにする。

640px 未満では、サムネイルを幅 112px に縮めてカードの内側の右上に置き、説明を出さない。上に全幅で置くと 1 枚が 270px ほどになり、画面の 4 割を占める。説明は 1 行に 12 文字ほどしか入らず、書き出しの数語で切れて中身が読み取れない。

残るのはタイトル 2 行とホスト名で、積むと約 62px になる。幅 112px のサムネイルの高さ 59px とほぼ同じなので、タイトルの上端とホスト名の下端をサムネイルの上下にそろえる。カードの高さは、タイトルの行数を問わず 85〜88px になる。

サムネイルの `alt` は空にする。リンクの名前はタイトルの文字で足りていて、`alt` にもタイトルを入れると読み上げで 2 回続く。

...
今後入れたい機能が 3 つあります。OGP の内製化、ダークモード、TOC の生成です。この 3 つはいずれも、現状のコードの上にそのまま載せられません。載せようとすると、同じ処理を 2 箇所に書くか、原理的に動かないかのどちらかになります。

土台になっている問題は 5 つです。

**MDX の経路が 2 つあり、片方がワークアラウンドで出来ています。** 記事本体はビルド時に変換されます。一方で一覧ページの抜粋だけは別経路を通り、ファイルを文字列として読んで区切り文字で切り、実行時にコンパイルしてから画像パスを文字列置換で直しています。この経路には remark と rehype のプラグインが適用されていません。TOC の見出し抽出を入れるなら、2 つの経路の両方に入れる必要があります。
...
**色がビルド時に焼き込まれる静的な定数です。** 実行時に差し替える余地がないため、この形のままではダークモードが入りません。名前も見た目を指しているので、ダークモードでは `white` が黒を意味することになります。

**ビルドが外部サービス 2 つとリンク先サイト 196 件の死活に依存しています。** 記事を 1 文字も変えていなくても、どれか 1 つが落ちればデプロイが失敗します。

**テストが 1 つもありません。** 検証手段は `pnpm build` が通るかどうかだけです。この後に挙げるバグ (一覧の抜粋の欠け、OG 画像の 404、無効な CSS、前後記事のタイトル引き) がどれも本番に残っているのは、どれもビルドを通してしまうからです。
...
前後記事の探索はタイトルで `findIndex` しているため、同名の記事を書いた時点で壊れます。さらに見つからなかった場合 `findIndex` は -1 を返し、その結果として一覧の先頭の記事が「前の記事」として黙って表示されます。エラーにならないので気づけません。
...
日付の扱いにはもう 1 つ問題があります。`parseDate` は `parse(str, 'YYYY-MM-DDTHH:mm:ss', 'Asia/Tokyo')` と書かれていますが、この第 3 引数が効いておらず、返る Date はビルドマシンの TZ に依存します。同じ frontmatter から、ローカル (JST) と CI (UTC) で 9 時間ずれた瞬間が得られます。`YYYY/MM/DD` の表示はパースと整形が同じズレで相殺されるため無事ですが、機械可読な日時を出す箇所では表面化します。
...
| A-10 | `parseDate` の `Asia/Tokyo` 指定が効かず、返る Date がビルドマシンの TZ に依存する | `app/lib/time.ts:4` | 2 |

### MDX の二重経路

一覧ページの抜粋は、記事本体とはまったく別の経路で描画されます。`PostSummarySection` が mdx ファイルを `fs.readFileSync` で文字列として読み、`{/* <!--more--> */}` で切り、`split('---')[2]` で frontmatter を落とします。それを `MarkdownRenderer` が実行時に MDX としてコンパイルします。

この経路には remark と rehype のプラグインが一切適用されていません。プラグインを入れるとエラーになるためコメントアウトされています。そのため一覧ページでは表とシンタックスハイライトと Mermaid が効きません。画像パスは、コンパイル済みの JavaScript 文字列に対する `replaceAll('./', imgBaseUrl)` という無差別置換で解決しています。本文に `./` が現れれば壊れます。

`split('---')[2]` は本文中の水平線でも切れます。`filter-data-structure` の抜粋が冒頭の挨拶までしか出ていないのは、これが原因です。

TOC はこの経路の上に乗ります。見出しの抽出とアンカーの付与を両方の経路に入れる必要があり、しかも片方はプラグインを受け付けない状態です。
...
色は `app/styles/color.ts` の静的な定数で、hono/css がビルド時にリテラル値を CSS へ焼き込みます。実行時に差し替える余地がないため、この形のままではダークモードが入りません。
...
テストは 1 つもありません。`app/lib/posts.ts` はほぼ純粋関数の塊で、ページネーションとタグの集約と slug の変換は、本来どれもテストしやすい部類です。ここまでに挙げたバグがどれも本番に残っているのは、どれもビルドを通してしまい、他に検証手段がないからです。

| ID | 内容 | 場所 |  |
| --- | --- | --- | --- |
| E-1 | `blog-api.p1ass.com` の取得に失敗すると、ビルドが落ちずに壊れたページが出力される。55 記事 196 箇所を毎回取得 | `app/lib/ogp.ts:50` | 6 |
...
目的は、本番で現に壊れているものを止めることです。OG 画像の 404、Pocket のデッドリンク、表示されない border、前後記事の誤リンクは、いずれも今この瞬間に読者から見えています。構造には手を入れないので、以降のステップと衝突しません。
...
目的は 2 つあります。1 つはステップ 3 と 4 を安全に進めることです。この 2 つは既存の挙動を保ったまま構造を組み替えるため、テストが先にないと壊れた事実に気づけません。もう 1 つは frontmatter の揺れを二度と入れないことです。日付フォーマットが 2 本で揺れているのも、`categories` が配列のままなのも、書いた時点で誰も止めなかったからです。
...
**RSS の `pubDate` が壊れていました。** 12 時間表記の `hh` を使っていたため、13 時以降に投稿した記事の時刻がずれていました。オフセットも `+00:00` と RFC 822 非準拠でした。`2025-07-14T19:00:00` の記事が `Mon, 14 Jul 2025 07:00:00 +00:00` として配信されていた状態です。A-9 の修正にあわせて直しました。
...
目的は、記事を見分けるキーを 1 つに決め、URL をそこから導出する形にすることです。カテゴリとタグの実装が 1 つになるため、以降の変更が片方だけに入る事故がなくなります。ステップ 7 で目次やテーマを足すとき、触る場所が 1 箇所で済むかどうかがここで決まります。
...
目的は、MDX の経路を 1 つにすることです。抜粋のバグが構造ごと消え、一覧ページでも記事本体と同じ描画になり、TOC の実装先が 1 箇所になります。

このステップが今回の中心です。ここを飛ばして TOC に進むと、見出しの抽出を 2 つの経路へ書くことになり、しかも片方はプラグインを受け付けません。
...
**E-1 の壊れ方が想定より悪いことが分かりました。** 計画には「外部サービスが落ちるとビルドが落ちる」と書いていました。しかし実際にはビルドが落ちません。`fetchOgp` が投げると SSG がそれを握りつぶし、`Internal Server Error` という中身の `index.txt` を書き出します。今回のビルド中に実際に発生し、`/posts/java-catch-up/` が壊れたページとして出力されました。次のビルドでは成功したため、断続的な失敗です。ステップ 6 でフォールバックを入れるときは、失敗を握りつぶさないことも条件に加えます。
...
目的は、実行時に色を差し替えられる形へ変えることです。ダークモードの前提であり、これがない限りステップ 7 へ進めません。
...
目的は、ビルドを自分の管理下に置くことです。今は外部サービス 2 つとリンク先サイト 196 件のどれかが落ちるだけでデプロイが失敗します。記事を書いた本人にはどうにもできない理由で公開が止まる状態をなくします。
...
目的は、機能を足すことです。ここまでの 6 ステップは、すべてこの 2 つを無理なく載せるための土台でした。

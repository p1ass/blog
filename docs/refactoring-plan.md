...
**記事を何で同定するかが決まっていません。** `id`、`permalink`、`fullFilePath` が並立していて、前後記事の探索はタイトル一致で行われます。カテゴリとタグは同じ構造をしているのに別々に実装されていて、片方にだけバグが残っています。この曖昧さがある限り、記事一覧まわりに機能を足すたびに実装が 2 箇所に増えます。
...
`app/lib/posts.ts` は glob のキーから `id`、`permalink`、`fullFilePath` の 3 つを個別の文字列操作で作ります。どれが同定キーなのかコードから読み取れず、`PostPagination` は受け取った `id` にさらに `.replace(/\/index\.mdx$/, '')` をかけ直しています。
...
目的は、記事の同定キーを 1 つに決め、URL をそこから導出する形にすることです。カテゴリとタグの実装が 1 つになるため、以降の変更が片方だけに入る事故がなくなります。ステップ 7 で目次やテーマを足すとき、触る場所が 1 箇所で済むかどうかがここで決まります。

#!/usr/bin/env node
// 記事の OG 画像を作る。ビルドの最後に回し、dist/posts/<slug>/og.png へ書き出す。
//
// 以前は og-image.p1ass.com という別のサービスが、共有されるたびにその場で描いていた。
// 記事の数は増えても 1 年に 10 本ほどで、しかもタイトルは公開したあと変わらない。
// ビルドのときに 1 度描けば済むものを、サービスを 1 つ立てて持ち続ける理由がない。
//
// frontmatter に ogImage がある記事は、そちらを使うので描かない。
//
// 描く中身は前のサービスと揃えてある。1200 × 630、白地、中央にタイトル、左下にサイト名と URL、右下にアイコン、下辺に accent の帯。
// 色はテーマの明るい側から引く。OG 画像は SNS の白い枠の中に出るので、読者のテーマには従わない。
...
// satori に任せると、幅が尽きた場所でそのまま折れて「参加し|て」のような切れ方になる。
// keep-all にするとゼロ幅スペースでも折らず、画像の外へはみ出す。

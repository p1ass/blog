...
| ビルド結果のチェック | 記事が全部出ていること、本文に知らない要素が出ていないこと | `pnpm build` の最後 |
...
2 つめは記事本文に知らない要素が出ていないこと。記事は MDX なので、生の HTML を書けば何でも入る。一覧は同じファイルの `allowedElements` にあり、要素ごとにスタイルの置き場所を書いてある。**要素を足すときは、先にスタイルを当ててから一覧に足す。** 順序を逆にすると、このチェックが「見た目を揃える」という役目を失う。

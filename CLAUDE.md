...
改行はこちらで決めて `\n` として渡す。satori に任せると幅が尽きた場所で折り返して「参加し|て」のような切れ方になり、`word-break: keep-all` にすると今度は折り返さずに画像からはみ出す。

どこで折るかは、行の余りの 2 乗と切れ目の不自然さを足した費用が最も小さくなる組み合わせを選ぶ。切れ目の候補は budoux の文節と kuromoji の語で、文節の切れ目を優先する。貪欲に詰めるだけだと、最後の行に「試した」の 3 文字だけが残る。

文字の大きさは、タイトルだけを satori に描かせて高さを測り、収まる中でいちばん大きいものを選ぶ。測った高さは行数の確かめにも使い、幅の概算が外れて satori に折り返されていたら、幅を狭めて組み直す。
...
- **Mermaid の図はビルドのたびにわずかに動くことがある。** `rehype-mermaid` はビルド時に Playwright で文字幅を測って座標を決める。同じコンテナの中でも 30 ピクセル程度の差が出た。落ちたときは差分の位置を見て、図の中だけなら撮り直す。
...
`textlint/plugins/` は `file:` で参照している。pnpm が `node_modules` へコピーするため、直したら `pnpm install` を回すまで textlint 側に反映されない。直したのに挙動が変わらないときはこれを確かめる。コメントの拾い方は `textlint/comment-ja.test.ts` で見ている。プラグインを触ったときはここに足す。

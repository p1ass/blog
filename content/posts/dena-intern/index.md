...
date: 2019-08-27T18:00:00+09:00
draft: false
description: 
categories:
- インターン
...
この度、8月23日~25日に開催されたDeNAのエンジニアサマーインターン プロダクト開発コースにサーバーサイドエンジニアとして参加してきました！

「3日間で最高のアプリに作り直す」というミッションが課せられていたインターンでしたが、最大限の力で最高のアプリに作り直し、**無事優勝することが出来ました🎉**
...
来年申し込んでみようかなと考えている人にも役に立ちそうな内容を盛り込んでいるので、是非読んでいってください。
...
このコースはDeNAが用意した伸びしろのあるアプリ、言い換えれば**クソアプリ**を3日間で良いアプリに作り直すという内容のインターンです。
...
そんな中、3月の終わり頃にDeNAのインターンの募集が始まっていることに気づき、速攻で申し込みました。
...
申し込んでから10日で合格が決まったのでかなり爆速でした。
人によっては面接から合格までかなり待ったという話があったので、運が良かったんだと思います。
...
面接の内容は他でも聞かれるような質問が多かったのですが、「プログラミングを始めたきかっけ」を結構根掘り葉掘り聞かれたのが印象に残っています。

「中学のころにC言語でCUIのテトリスを作ったりして遊んでました」と回答したのですが、「どんなライブラリ使ってたの？」とかを聞かれ、**「7年近く前のことなんか覚えてないよ！」**と思いながら面接してました。

ここで少し後輩に向けてのアドバイスをしておくと、サーバーサイドをやりたいなら
...
クライアント側は使用するライブラリ等も細かく指定されていたのですが、サーバーは**「GoでMySQLでCentOSだよ〜(o・∇・o)」**程度の情報量しかありませんでした。
しかし、キックオフイベント後の懇親会で、ISUCON8の問題作成者が今回のインターンの問題作成に関わっているということを聞き、主にパフォーマンスチューニングの方面を重点的に調べることにしました。
...
調べた内容は全てHackMDにまとめていたのですが、これらはインターンの当日に**とても重宝しました**。
特にDBの調査は今まであまりしたことがなく、何も覚えていなかったので、これがなかったらスムーズに調査することは出来なかったと思います。
やっぱり予習って大事ですね。
...
なかなか鬼畜な設定ですが、実際はこんなことないらしいです。


まず、実際のアプリの挙動を確認して駄目なところをピックアップしていき、重要度と工数でタスクを分類分けしていって、できるものから取り組んでいくことにしました。
...
を行いました。
途中で**AWSが死ぬ**などのアクシデントがありましたが、落ち着いて自分ができることをこつこつやっていきました。この時点でサーバー側のアプリケーションに仕込まれていたクソ仕様はあらかた片付いたので、明日は新機能も着手できそうという感じでした。
...
そのため、**「ユーザーのリクエストがそのままDBに貫通しないようにする」**という基本的な方針のもと、各レイヤーでキャッシングしていくことにしました。

具体的には、コメントの取得は、

1. コメントのAPIレスポンスをCloudFrontでTTL5秒でキャッシュする
1. Appサーバーのインメモリでコメントのレスポンスをキャッシュする
...
この実装によりインメモリで持つコメントの内容は各サーバで変わってしまいますが、クライアントは1万コメントを1秒間で表示することは不可能なので、コメントを同期することは諦めました。

しかし、クライアントがポーリングでコメントを取得するとき、LBによって毎回違ったサーバにアクセスしてしまうと、コメントの流れ(擬似的な部屋、ニコ生のアリーナや立ち見でそれぞれできるコメントの流れ)のようなものが失われてしまうと考えました。

そこで、HTTPヘッダーを使って**「あるクライアントは必ずある1つのサーバからコメントを取得する」**ようにしました。
発想としてはクライアントLBに近いと思います。これによりコメントの流れが不自然になることを防ぐことができました。

また、上で説明したキャッシュ戦略を導入したことで、質問チャンネルではMySQLへのアクセスをほとんどなくすことができました。一部各サーバー間で共有する必要があるデータはRedis経由で同期するようにし、こちらも各ユーザのリクエストが直接Redisに行かないように、各Appサーバーでキャッシュするようにしました。
...
この日はかなりサーバーサイドのタスクが多く、クライアント向けの実装はあまり出来なかったのですが、クライアントチームはサーバーなしでもできることを着実に進めてくれていました。  
いつの間にかアプリにお絵かき機能が追加されていたときは流石に驚きました🤭
...
サーバーサイドはインフラ構成や計測結果、その他工夫などを簡潔に1枚のスライドにまとめて発表しました。

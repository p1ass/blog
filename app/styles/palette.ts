...
// アクセント。色相 215°。
// 500 は OG 画像の下辺のボーダーにも使っている事実上のブランドカラーなので、値を変更しない。
// 300 だけ彩度が高い。暗い地に置くと同じ彩度でも色みが弱く見え、青がくすんで感じられるため、この段だけ 47% から 80% へ上げてある。
// 900 は暗いテーマの Note の面。50 が白へ混ぜた青なのに対して、こちらは neutral の 950 へ混ぜた青にあたる。
export const accent = {
  50: '#e8f5fe', // 既存の accentSurface
  100: '#cfe2fc',
  200: '#adc7eb',
  300: '#7eaef1', // 暗いテーマのリンク。彩度は 80% で、この段だけ他より高い
  400: '#6891ca',
  500: '#4172b5', // 既存の accent
  600: '#365e96',
  700: '#2b4b78',
  800: '#223959',
  900: '#16263b', // 暗いテーマの Note の面。accent の 300 が上で 6.70 対 1
} as const

// 注意を促す囲みの色。色相 35 度。
// accent と同じ色相では「いつもと違う」が伝わらないので、ここだけ別の系統を持つ。
// 段は使うぶんだけ置く。明るいテーマと暗いテーマそれぞれの、面と、その上に置くアイコンの 4 つで足りる。
export const warning = {
  50: '#fff4e5', // Note の面
  300: '#f0a742', // 暗いテーマの Note のアイコン。面の上で 7.19 対 1
  600: '#a35b00', // Note のアイコン。面の上で 4.71 対 1
  900: '#342614', // 暗いテーマの Note の面
} as const

// 補足や助言の囲みの色。色相 145 度。
export const tip = {
  50: '#e8f6ec', // Note の面
  300: '#66cc90', // 暗いテーマの Note のアイコン。面の上で 7.36 対 1
  600: '#1f7a3d', // Note のアイコン。面の上で 4.81 対 1
  900: '#142e1f', // 暗いテーマの Note の面
} as const

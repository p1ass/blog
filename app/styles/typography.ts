...
// 本文 17px を基準に、比 1.2 の等比。1 行が全角 44 文字になる幅 (760px) と合わせて、日本語の長文が読みやすい範囲に入れた。
...
//
// リグレッションテストのコンテナではこの並びのどれも存在しないため sans-serif に解決される。その解決先は vrt/fonts/local.conf で固定してある。
//
// フォント名を引用符で囲まないのは、hono/css が補間した値の " をエスケープし、\" という不正な CSS になるため。
// CSS のフォント名は識別子の並びとして書けるので、引用符は要らない。
export const fontFamily = {
  // 和文を先に置く。欧文だけのフォントを先に置くと、和文がそのフォントのフォールバックに委ねられ、環境によって別のフォントで描かれる。
  body: [

...
// 文章の中ではない、それ自体が 1 つのコンポーネントとして置かれるリンク。前後の記事へのリンクとフッターがこれを使う。
//
// 独立して並んでいるので、押せることは位置と色で分かる。平常時に下線を出すと、文字の下に線が並ぶだけになる。
// 下線そのものは常に引いておき、色を transparent から currentColor に変える。text-decoration の有無は補間できず、hover したとたんに線が現れて文字が跳ねて見える。

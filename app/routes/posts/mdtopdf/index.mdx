...
![PDFの例1](./example-1.jpg)

![PDFの例2](./example-2.jpg)

![PDFの例3](./example-3.jpg)

依存は全て Docker イメージにまとめてあるので、 **Docker さえあれば、ローカルへ何もインストールせずに、誰でも簡単に美しい実験レポートを作成できます**。
皆さん是非試してみてください。
...
<ExLinkCard url="http://mizti.hatenablog.com/entry/2013/01/27/204343"/>
...
`-N` オプションを付けると、セクション番号を自動で付与してくれます。

`-F pandoc-crossref` では[pandoc-crossref](https://github.com/lierdakil/pandoc-crossref)という相互参照のためのフィルタを使っています。
...
<ExLinkCard url="https://github.com/lierdakil/pandoc-crossref"/>
...
保存ごとの PDF 変換は簡単で、次のスクリプトを使っています。

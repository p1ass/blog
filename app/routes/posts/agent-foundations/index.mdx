...
Model は、次に Tool を呼ぶのか、ユーザーに回答を返すのかを判断する LLM です。

Instructions は、Agent の役割や守るべき制約を書いた指示で、API ではシステムプロンプトとして渡します。

Tool は、前のセクションで見た Tool Call で呼び出される関数や API です。
Model には名前、説明、引数の JSON Schema を渡し、実行はアプリケーションが担います。

後で実装する天気の Agent で言えば、Model は `gpt-6-sol`、Instructions は「天気を答えるアシスタントとして振る舞い、天気は必ず `get_weather` で調べる」という指示、Tool は `get_weather` にあたります。
...
コードの全体は [GitHub](https://github.com/p1ass/blog/blob/main/app/routes/posts/agent-foundations/sdk-agent/openai_agent.py) にプッシュしています。
...
ループも Tool の実行も、すべて Python のプロセスの中で動きます。ターン数が `max_turns` を超えると、`Runner` は `MaxTurnsExceeded` を throw します。
...
Claude Agent SDK には `Agent` クラスがありません。Model、Instructions、Tool は `ClaudeAgentOptions` にまとめ、`ClaudeSDKClient` に渡す仕組みです。
...
コードの全体は [GitHub](https://github.com/p1ass/blog/blob/main/app/routes/posts/agent-foundations/sdk-agent/claude_agent.py) にプッシュしています。
...
環境変数 `OPENAI_API_KEY` と `ANTHROPIC_API_KEY` を設定し、それぞれの版を実行すると、次のようなログが出力されます。
...
どちらの版も、1 ターン目で東京と大阪の 2 つの Tool Call を 1 回の応答で返しています。2 つの結果をまとめて返すと、2 ターン目で Tool Call を含まない最終出力が返り、ループが終わります。最終出力の文面はモデルによって異なりますが、ループの動きは同じです。

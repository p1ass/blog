...
date: 2026-09-24T22:15:00+09:00
...
ループも Tool の実行も、すべて Python のプロセスの中で動作します。
...
Claude Agent SDK には `Agent` クラスがありません。代わりに、`ClaudeAgentOptions` というクラスが Model、Instructions、Tool を持ち、`ClaudeSDKClient` に渡す仕組みになっています。
...
題材はこれまでと同じく `get_weather` を持つ Agent です。「東京と大阪の天気を教えてください」と頼み、最終出力を返すまで Agent Loop を回します。
...
この記事では、OpenAI のドキュメントをもとに、Agent を Model、Instructions、Tool の3要素と、Tool Call を繰り返す Agent Loop として整理しました。

...
この記事では、実際に検出できる単語の例を見せたうえで、作った背景や使い方、形態素解析を使った検出の仕組みを紹介します。
...
まず手始めに、作成したプリセットを使って、AI が書いた技術記事から AI っぽい単語を検出してみます。テーマは Claude Code と Codex の比較で、冒頭は次のような文章です。
...
最近は文章を書くときに AI を使う機会が増えましたが、出てくる日本語の表現が読みにくかったり、不自然と感じる機会が多くありました。

{/* <!-- textlint-disable ai-words-ja/no-ai-words --> */}

特に気になるのが、英語を直訳したような表現です。
先ほどの例のように「設計が効いてくる」「テストを実測する」「原因を切り分ける」「深く踏み込む」といった単語が、人が書いた技術文書ではあまり見かけない頻度で出てきます。
...
そこで、「決定論的に検知できるツールを作ればよいのでは？」と考え、すでにエコシステムが発展している textlint をベースに、AI が多用する単語の辞書を持ったプリセットを作ることにしました。
...
今回作ったプリセットは単語そのものを見るので、両方を併用できる形にしました。
...
Claude Code では、hooks の `PostToolUse` を使います。
...
例えば、検査や部品はソフトウェアエンジニアリングに関する文章では見かけない表現ですが、製造業や機械では当たり前の表現でしょう。
...
非常に興味深い内容で、生成 AI 特有の日本語文章のクセを分析した記事は見かけたことがなかったため目から鱗な内容でした。

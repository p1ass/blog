...
この記事では、実際に検出できる単語の例を見せたうえで、作った背景や使い方、形態素解析を使った検出の仕組み、辞書を作った経緯を紹介します。
...
手始めに、作成したプリセットを使って、AI が書いた技術記事から AI っぽい単語を検出してみます。テーマは Claude Code と Codex の比較で、冒頭は次のような文章です。
...
ai-generated-text.md
    ...
    5:6   error  主題を 5 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
...
ai-generated-text.md
   ...
   26:4   error  主題を 3 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
   ...
   38:6   error  主題を 5 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
   43:4   error  主題を 3 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
   ...
   73:6   error  主題を 5 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
   ...
   84:4   error  主題を 3 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
  ...
  113:6   error  主題を 5 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
  119:9   error  "踏み込む" はAIが書いた文章で多用される表現です。別の表現に言い換えられないか検討してください。      ai-words-ja/no-ai-words
  120:4   error  主題を 3 文字示しただけで読点を打っています。読点を外すか、文を組み替えられないか検討してください。  ai-words-ja/no-short-topic-comma
...
文章全体では、38 件の指摘が検出されました。「依存関係を照合する」「テストを実測する」のように、「むむ？」と思う表現を拾えていると思います。

{/* <!-- textlint-enable ai-words-ja/no-ai-words --> */}

## textlint のプリセットを作った背景

最近は文章を書くときに AI を使う機会が増えましたが、出てくる日本語が読みにくかったり、不自然に感じたりすることが多くありました。
...
なお、AI らしい文章を検出するプリセットには [@textlint-ja/textlint-rule-preset-ai-writing](https://github.com/textlint-ja/textlint-rule-preset-ai-writing) が既にあります。
こちらは太字とコロンを組み合わせた箇条書きや、コロンでコードブロックを導入する書き方のように、主に文章の構造を対象にしています。
今回作ったプリセットは主に単語や言い回しを見るので、両方を併用できる形にしました。
...
プリセットには、辞書に載せた単語や言い回しを見つける `no-ai-words` と、短い主題の後に打った読点を見つける `no-short-topic-comma` の 2 つのルールが入っています。どちらも文章を形態素解析して得たトークン列を使います。
...
AI が書いた文章では単語のほかに、「重要なのは、」「それは、」のように短い主題を示してすぐ読点を打つ書き方も気になっていました。
先ほどの例でも、5 行目の「重要なのは、」が指摘されています。

`no-short-topic-comma` は係助詞の「は」の直後に読点があり、文の始まりからその「は」までが 5 文字以内のときに指摘します。
品詞を見て判定しているので、「しかし、」のような接続詞の後の読点は指摘しません。

指摘する長さは `maxLength` で変えられます。
...
textlint で指摘しておけば同じ文章なら同じ結果が返るので、文章のクオリティーを一定に保つうえで役立つと思います。辞書に足したい単語があれば、Issue や Pull Request で教えてください。

...
内蔵の辞書は、私が読んできたソフトウェアエンジニアリングの技術文章をもとに作ったので、載っている単語もその分野に偏っています。
...
用意したファイルのパスを `dictionaryPath` に指定します。相対パスは設定ファイルのディレクトリから解決されます。
...
何も指定しなければ、ファイルの辞書は内蔵の辞書に追加されます。
分野が離れていて内蔵の辞書の指摘が合わない場合は、自分の辞書だけで検出するように `dictionaryMode` に `"override"` を指定してください。

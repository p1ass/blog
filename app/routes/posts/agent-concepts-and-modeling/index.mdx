...
OpenAI と Anthropic はそれぞれ別々に Agent SDK を開発していますが、2 つの SDK の中にある Agent の捉え方はほぼ同じです。どちらも「モデル、指示、Tool を持ち、Tool 呼び出しがなくなるまでループを回す」という形で Agent を組み立てています。この形を最もまとまった形で説明しているのが、OpenAI が 2025 年 4 月に公開したガイドです。

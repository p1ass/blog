...
OpenAI と Anthropic はそれぞれ別々に Agent SDK を開発していますが、2 つの SDK の中にある Agent の捉え方はほぼ同じです。どちらも、OpenAI がガイドで示した「モデル、指示、Tool を持ち、Tool 呼び出しがなくなるまでループを回す」という形で Agent を組み立てています。
この記事では、まず Agent を理解する前提となる Tool 呼び出しの仕組みを確認し、OpenAI のガイドから Agent のモデルを整理します。そのうえで、OpenAI Agents SDK と Claude Agent SDK がそのモデルをどう表現しているのかを比較し、最後に Go で API を直接呼び出す最小限の Agent を実装します。
...
2023 年に OpenAI が function calling として API に追加し、現在では Anthropic (tool use) や Google も同様の仕組みを提供しています。この記事では、これらを総称して Tool と呼びます。

モデル自身は Tool を実行しません。
モデルはどの Tool をどんな引数で呼び出すかを、構造化データとして返すだけです。アプリケーション側のコードがその Tool を実行し、結果をもう一度モデルに渡すことで、モデルはその結果を踏まえて応答を生成します。
...
_Tool 呼び出しの 1 往復。モデルは呼び出しを指定するだけで、実行はアプリケーションが担う_

この往復を 1 回で終わらせず、モデルが Tool 呼び出しを返さなくなるまで繰り返すと、Agent のループになります。

## OpenAI のガイドが示す Agent のモデル

OpenAI は 2025 年 4 月に公開したガイドで、Agent を次のように説明しています。
...
### Model、Instructions、Tools の 3 要素
...
ガイドでは Agent の 1 回の実行を Run と呼び、終了条件を満たすまで回すループとして説明しています。
終了条件は、特定の Tool 呼び出し、特定の構造化出力、エラー、最大ターン数への到達の 4 つです。
...
どちらも、最初の節で見た Tool 呼び出しの往復を、LLM が自分で判断しながら繰り返すループを Agent の中心に置いています。
...
- モデルに指示と使える Tool の一覧を渡す
- モデルが Tool 呼び出しを返したら、コードがそれを実行し、その結果を会話に含めてもう一度モデルを呼ぶ
- モデルが Tool 呼び出しのない応答を返したら終わる。無限に回らないよう最大ターン数を設ける
...
OpenAI Agents SDK は、2025 年 3 月に公開された SDK です。それまで実験的に公開されていた Swarm の後継にあたります。公式ドキュメントでは Agent を次のように説明しています。
...
`Runner.run_sync` は 1 ターンごとにモデルを呼び出し、その出力に応じて次の 3 つに分岐します。

```mermaid
flowchart TD
  start([Runner.run]) --> invoke[現在の Agent のモデルを呼ぶ]
  invoke --> kind{出力の種類}
...
Handoff は、モデルからは普通の Tool に見えます。
...
モデルが `transfer_to_refund_agent` を呼び出すと、`Runner` は現在の Agent を Refund Agent に差し替えてループを続けます。
ガイドでは、問い合わせを振り分ける Triage Agent の例を使っています。
...
Claude Agent SDK は、2025 年 9 月に Claude Code SDK から改名された SDK です。公式ドキュメントでは Agent を次のように説明しています。
...
Claude Agent SDK には `Agent` クラスがありません。モデル、指示、Tool は `ClaudeAgentOptions` にまとめ、`ClaudeSDKClient` に渡します。
独自の Tool は、プロセス内で動く MCP サーバーとして渡します。モデルからは `mcp__<サーバー名>__<Tool 名>` という名前の Tool に見えます。OpenAI Agents SDK の例と同じ Agent は次のようになります。
...
`model`、`system_prompt`、Tool の 3 つが、ガイドの Model、Instructions、Tools にあたります。`allowed_tools` は Tool を実行するたびに許可を求めないための設定です。
...
委譲にはサブ Agent (subagent) を使います。`ClaudeAgentOptions` の `agents` に `AgentDefinition` を渡すと、モデルは `Agent` という Tool を通してサブ Agent を呼び出します。
...
`AgentDefinition` は Claude Agent SDK で 1 つの Agent を表す唯一の型です。そのフィールドは `prompt` (指示)、`tools`、`model` と、呼び出す側のモデルに見せる `description` で、ここにもガイドの 3 要素がそのまま現れています。
...
| --- | --- | --- |
| モデル | `Agent.model` | `ClaudeAgentOptions.model`、`AgentDefinition.model` |
| 指示 | `Agent.instructions` | `ClaudeAgentOptions.system_prompt`、`AgentDefinition.prompt` |
...
題材として、固定の天気を返す `get_weather` という Tool を 1 つだけ持つ Agent を作ります。「東京と大阪の天気を教えてください」と頼み、Tool 呼び出しがなくなるまでループを回します。
...
Anthropic 版では、指示を `System` に、Tool の定義を `Tools` に渡します。Tool の引数は JSON Schema で定義します。
...
モデルの応答と Tool の実行結果を `params.Messages` に蓄積し、次のリクエストで会話全体を送り直します。
Tool の実行結果は、`tool_result` ブロックとして user のメッセージに入れて返します。

### OpenAI Responses API 版

OpenAI 版では、指示を `Instructions` に、Tool の定義を `Tools` に渡します。`runTool` は Anthropic 版と同じです。
...
OpenAI Agents SDK と Claude Agent SDK は、どちらも OpenAI のガイドが示した Agent のモデルで読み解けます。モデル、指示、Tool を持ち、Tool 呼び出しがなくなるまでループを回し、別の Agent への委譲も Tool 呼び出しとして表します。
違いは OpenAI Agents SDK が Agent をオブジェクトとして組み立てて `Runner` で動かすのに対し、Claude Agent SDK は Claude Code という完成した Agent を設定して動かす点です。

2 社の捉え方が似ている理由は、各社の API の Tool 呼び出しが同じ形をしているためだと私は考えています。モデルが Tool 呼び出しを返し、コードがそれを実行して結果を返すという往復は、OpenAI、Anthropic、Google の API で共通です。Agent のループはこの往復を繰り返すだけなので、その上に組み立てる SDK の形も似てきます。
実際、ループそのものは Go で数十行のコードで実装できます。SDK を選ぶときは、Handoff や Guardrails、トレースなど、ループに含まれない機能を比べることになります。

2026 年に入り、2 社は [Claude Managed Agents](https://www.anthropic.com/engineering/managed-agents) や Agents API で、実行環境や状態といったループの外側まで含めて Agent をモデル化し始めています。この話と、ループのどこまでをコードで固定し、どこから LLM に任せるかは、今後の記事で扱う予定です。

...
2 社の SDK の概念を同じ構成に対応付けられること、そして SDK がどの処理を代わりに担っているのかを見分けられることが、この記事のゴールです。
...
2 社の Agent の形が似ている理由は、Tool Call の往復が各社の API で同じ形をしているためだと私は考えています。
Go の実装で見たとおり、この往復を繰り返す Agent Loop は 60 行ほどで実装できます。SDK ごとの差は、Guardrails やコストの上限、コンテキストの圧縮といった、ループの外側で担う機能に表れます。

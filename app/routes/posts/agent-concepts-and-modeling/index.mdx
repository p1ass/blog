...
OpenAI と Anthropic はそれぞれ Agent SDK を開発していますが、2 つの SDK の中にある Agent の捉え方はほぼ同じです。
どちらも「Model、Instructions、Tool を持ち、最終出力を返すまでループを回す」という形で Agent を組み立てています。
OpenAI が公開しているガイドには、この形が最もよくまとまっています。
...
この記事のゴールは、2 社の SDK の概念を同じ構成に対応付け、SDK がどの処理を代わりに担っているのかを見分けられるようになることです。
...
OpenAI のガイドは、Model、Instructions、Tool を持ってループを回すという形を、それまでの API や SDK での使われ方からまとめたものです。ガイドでは Agent を次のように説明しています。
...
終了条件は特定の Tool Call、特定の構造化出力、エラー、最大ターン数への到達の 4 つです。この記事では、最終出力と最大ターン数の 2 つに絞って扱います。
...
以降ではこのまとめを「Agent の構成」と呼び、2 つの SDK をこれに当てはめて読んでいきます。
...
`Agent` は設定を持つだけのオブジェクトで、ループを回すのは `Runner` です。
...
独自の Tool は、プロセス内で動く MCP サーバーとして登録します。Model からは `mcp__<サーバー名>__<Tool 名>` という名前の Tool に見えます。OpenAI Agents SDK の例と同じ Agent は次のようになります。
...
独自の Tool と hooks、Tool の実行を許可するかを決めるコールバックだけが Python 側で動きます。
...
題材はこれまでと同じ `get_weather` を持つ Agent です。「東京と大阪の天気を教えてください」と頼み、最終出力を返すまでループを回します。
...
API が違っても、Agent Loop の実装はほとんど同じです。
違いは Tool の実行結果を user のメッセージに入れるか独立したアイテムとして送るか、失敗を `is_error` で伝えるか文面で伝えるかといった、API ごとの細かな作法にとどまります。

どちらの API でも、Tool の定義を含めて main 関数は 60 行ほどに収まります。
SDK の価値の多くは、この最小のループが持たない機能にあります。
OpenAI Agents SDK なら Guardrails やトレース、Claude Agent SDK ならコストの上限 (`max_budget_usd`)、コンテキストの自動圧縮、セッションの再開などです。

{/* TODO: API キーを設定して実行し、実行ログを載せる */}

## おわりに

この記事では、OpenAI のガイドをもとに、Agent を Model、Instructions、Tool と、最終出力を返すまで回る Agent Loop に整理しました。
OpenAI Agents SDK と Claude Agent SDK はどちらもこの形をしています。違いは Agent をオブジェクトとして組み立てて `Runner` で動かすか、Claude Code という完成した Agent を設定して動かすかという点です。

Go の実装で見たとおり、Tool Call の往復を繰り返す Agent Loop は 60 行ほどで実装できます。SDK ごとの差は、Guardrails やコストの上限、コンテキストの圧縮といった、ループの外側で担う機能に表れます。

次の記事からは、ここで整理した要素とその周辺を 1 つずつ取り上げます。
次回は Tool の設計を扱い、その後はコンテキストの管理、ハーネス、マルチ Agent、制御とガードレール、観測と評価の順に進める予定です。どちらの SDK も別の Agent への委譲まで Tool Call として表している点は、マルチ Agent の回で詳しく扱います。

...
permissions:
  contents: read
  pull-requests: write # レビューを行うために必要
        ...
        with:
          ...
          direct_prompt: |
            ...
            gh api \
              ...
              --field event="COMMENT" \
              --field comments[0][path]="src/main.py" \
...
- 具体的な `gh api` コマンドの使い方をプロンプトに埋め込んで、Claude Code が `gh api` コマンドを使う確率を高くなるようにしています。

## おわりに

今のところ順調に動いています。
行指定でコメントを出してくれると、一気に人間のレビューっぽくなるのでおすすめです。

...
description: Agent 開発の連載の 5 本目。Multi-Agent を扱う。
category: 生成AI
tags:
  - Agent
...
ガイドは Multi-Agent の構成として 2 つのパターンを挙げています。
...
Agent の構成で見たループに、Handoff の分岐が 1 つ加わった形です。
...
委譲の仕組みも同じではなく、Handoff は OpenAI Agents SDK にしかありません。Manager と Handoff をどう使い分けるかは、Multi-Agent を扱う回で詳しく取り上げます。
